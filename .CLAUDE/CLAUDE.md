...
- **CONFIRMED BUG — `segment_tile()` returning only the largest region makes tile identity flip when two tiles share the frame** (found 2026-09-20 running the real belt footage through `development/process_conveyor_video.py`). This is the single most important thing found so far about the live pipeline, and it is **not** a trigger bug. On `data/WhatsApp Video 2026-08-26 at 7.27.04 PM.mp4`, **34.8% of frames (596/1715) hold two tile-sized regions**, and the two tiles' areas land within **~0.02% of each other** (measured: 26283 vs 26278 px at frame 613). Whichever is fractionally larger wins, so "the tile" flips between two physically different tiles on pure segmentation noise, and the reported centre teleports ~0.6 of the frame height in a single frame. Any centre-based trigger then fires spuriously: 12 crossings reported where 7 are real. The tell is *where* a crossing is recorded — a genuine one lands just past the line (`cy≈0.52`), an identity flip lands far from it (`cy≈0.83-0.86`), because the centre jumped there rather than travelled there. **`LineCrossingDetector`'s hysteresis cannot fix this and is not meant to** — it is a spatial deadband for jitter *at* the line, and a 0.6 jump clears any deadband (swept to 0.08, no change). The script's old hand-rolled 12-frame temporal cooldown *appeared* to give the right answer (9) purely by swallowing repeats inside 0.4 s — it was masking the flip. The real fix is upstream: track N regions and hold tile identity across frames (nearest-centre association or similar), which is a design decision, not a tweak. Everything downstream — tile counts, per-tile grades, snapshots, the model's second opinion — is untrustworthy whenever two tiles are in frame, so this blocks any multi-tile conveyor run.
- **The line-crossing trigger's tuning values are still rig-dependent and unvalidated** (updated 2026-09-20). The detector itself now *has* run against real conveyor footage (above) and behaved exactly as specified given its one-tile-in-frame precondition. But `trigger_line.position` depends on where the camera ends up mounted and `direction` on which way the belt runs, and `hysteresis: 0.02` was reasoned about (≈13 px at 640 px wide) rather than measured against real segmentation jitter — on the real clip it made no difference to the count at any value from 0.0 to 0.08, because the noise there was identity flipping, not jitter. Re-derive all three once the rig exists.
