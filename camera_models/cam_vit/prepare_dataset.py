...
cam_vit/train.py's TileGradeDataset expects.

Identical split logic to cam_yolo/prepare_dataset.py (same seed/val_fraction
by default -- see camera_models/cam_vit/config.yaml -- for a comparable split), kept as a
separate copy so this node's dataset can be regenerated/tuned independently.

Splits by SOURCE IMAGE GROUP, not by file (added 2026-08-26, when
source_dataset_dir was pointed at development/augment_dataset.py's offline-
augmented copy): that dataset's `dslr__DSC_0022.jpg`, `..._aug0.jpg`, and
`..._aug1.jpg` are near-duplicates of the same real tile photo (mild
rotation/shear/perspective of one another). Splitting by individual file
could put the original in train and an augmented copy of the SAME tile in
val (or vice versa) -- the model would then be evaluated on an image it
essentially already saw during training, inflating val accuracy without
that improvement reflecting real generalization. Grouping by the shared
`_aug<N>` -stripped stem keeps every variant of one source photo entirely on
one side of the split. This is a harmless no-op against the original,
...
data/, and camera_models/cam_vit/dataset/ can be regenerated (a different split seed, a
different val_fraction) by just deleting camera_models/cam_vit/dataset/ and rerunning this
script.
...
def main() -> None:
    config = load_config()
    source_dir = (CAM_VIT_DIR / config["source_dataset_dir"]).resolve()
