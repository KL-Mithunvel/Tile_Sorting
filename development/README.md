...
| `process_conveyor_video.py` | Runs the real conveyor-belt phone footage in `data/` (not the checkerboard calibration photos) through `camera_node`'s real `segment_tile()` **and its real `LineCrossingDetector`**, tuned with this clip's own belt-relative ROI (see its docstring for why a static crop and camera_node's calibrated HSV range both fail on this footage). Draws every tile-sized region — green for the one segmentation picked, amber for the runners-up it discarded — plus the trigger line, and flags the frame the tile's centre crosses it. Writes an annotated video + JSON crossing report to `development/output/` (the video is gitignored — it is ~23 MB and rebuilt on every run; the JSON is committed). **This is the script that found the two-tiles-one-identity bug** (2026-09-20): it reports how many frames held more than one tile-sized region, because while that count is non-zero the crossing count cannot be trusted — see its docstring and `TODO.md`. |
