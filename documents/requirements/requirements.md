...
| FR-23 | The camera station shall measure crack severity/length and missing-corner area directly rather than only classifying pass/fail, using classical image processing (grayscale conversion, edge detection, contour measurement) rather than a trained classification model — a size/severity number per defect, not just a category. | `camera_node/python/camera/crack_detection.py`, `corner_detection.py` (Decision, 2026-08-07 — an earlier pretrained crack-classification example only produced a grade, see `.CLAUDE/CLAUDE-LOG.md`) | Confirmed direction; detection thresholds are unvalidated placeholders pending real tile photos |
| FR-24 | The camera station shall isolate/segment the tile from its background before running defect detection, so crack/corner analysis runs only on the tile region and not the conveyor/bench background. | `camera_node/python/camera/segmentation.py` (2026-08-07) | Confirmed; segmentation HSV range is an unvalidated placeholder |
...
| FR-10a | Compute assignment: master = user's own PC; **camera node = Raspberry Pi 5 (16 GB), Raspberry Pi OS Desktop 64-bit**; acoustic node and pick-and-place controller = Arduino UNO Q each; conveyor/motion control = Arduino Mega. The camera/acoustic Pi-swap question is **closed** — camera took the Pi, acoustic keeps the UNO Q, on the grounds that only acoustic uses the UNO Q's real-time MCU half and only camera is compute-bound. | Automation_Architecture.md §5 (Decisions, 2026-07-10; pick-and-place added 2026-08-03; camera→Pi decided 2026-09-20, §5.2/§5.3) | Confirmed. Pi hardware in hand but not yet set up — no Pi-specific code has run. |
| FR-22 | Every station node shall have its own top-level repo folder. A node **whose code runs on an Arduino UNO Q** (acoustic, pick-and-place) shall be organized as an Arduino App Bricks-shaped project (`app.yaml` + `sketch/` + `python/`) so it can be opened/run as an App Lab app; a node on other hardware uses that platform's normal packaging and shall not carry App Bricks scaffolding describing hardware it lacks. One physical UNO Q and one Raspberry Pi 5 exist so far — the folder structure is a code-organization decision, not confirmation of a board per node. | Automation_Architecture.md §5.7 (Decision, 2026-08-03; amended 2026-09-20) | Confirmed structure. UNO Q nodes: App Lab runtime behavior (`arduino.app_utils.App.run()`) still not verified on hardware. Camera node: left the convention 2026-09-20 (`app.yaml` + `sketch/` deleted), now a systemd-deployed Python service (`camera_node/deploy/`) — written but never run. |
...
| FR-25 | The camera station's local monitor (FR-19) shall stream live video plus running tile count and per-tile crack/corner results over WiFi to a browser-based dashboard, reachable from any device on the local network rather than only the camera node itself. | `camera_node/python/camera/dashboard.py` (Decision, 2026-08-07) | Confirmed and implemented (Flask, MJPEG stream); not yet run against a real webcam feed end-to-end |
...
| FR-21 | The acoustic station shall detect tile arrival via a laser or ToF sensor, which triggers a ball-drop impactor via a dual-solenoid ARM+LOCK release mechanism (ARM raises the ball to drop height, LOCK releases it to free-fall onto the tile — neither solenoid touches the tile directly); the same Arduino UNO Q controlling the trigger shall also capture and process the resulting sound before sending the result to master. This supersedes the push-pull solenoid striker as the working plan (kept as background). | Charter §6.2 (Decision, 2026-07-10; release mechanism decided 2026-08-20); `documents/electrical/schematics/acoustic_station_wiring.md` | Confirmed, incl. release mechanism and pin assignments; ball mass/drop height and reload/reset method still open |
...
- FR-21's release mechanism is now decided (dual-solenoid ARM+LOCK, `acoustic_station_wiring.md`), but ball mass/drop height and the reload/reset method between tiles are still undecided — don't treat the acoustic tapping mechanism as fully finalized.
...
- **FR-22** — the App Bricks folder convention (`app.yaml`/`sketch/`/`python/`) applies to `acoustic_node/` and `pick_place_node/`, but `arduino.app_utils.App.run()`'s actual behavior has never been exercised against the lab UNO Q board — both `python/main.py` files are unverified stubs, not confirmed working App Lab entry points. `camera_node/` left the convention on 2026-09-20 and its `python/main.py` is a real systemd entry point instead (verified to start on the dev laptop; never run on a Pi).
- **FR-10a** — the Raspberry Pi 5 camera station is decided and the code is restructured for it, but no Pi has been imaged and nothing Pi-specific has executed: `camera_node/deploy/install_pi.sh`, the `tile-camera` systemd unit, and `camera/capture.py`'s `PiCameraCapture` were all written from documentation, not observation.
...
- **FR-23/FR-24** — every camera vision threshold (segmentation HSV range, Canny edge thresholds, crack length/aspect-ratio thresholds, corner fill-ratio threshold) is an unvalidated placeholder. The user has sample tile photos (crack/corner-break/good) from a different camera than the one that will actually be used, but they weren't available on this machine when the pipeline was built — re-tune against them, and ideally against photos from the actual target camera, before trusting any result.
- **FR-23** — crack "type" as currently reported is severity (minor/major) + measured length, not a defect taxonomy; corner detection reports that + how much is missing, not which corner. Both are deliberate scope cuts pending real photos, not oversights.
- **FR-25** — the dashboard has been smoke-tested via Flask's test client and a synthetic-image pipeline run, but never against a real webcam feed end-to-end (no camera hardware available to the assistant in this session).
